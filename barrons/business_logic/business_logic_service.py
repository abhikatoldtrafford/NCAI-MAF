...
from NewsAgents.application.services import PromptService
from NewsAgents.business.interfaces.business_logic_interface import BusinessLogicInterface
from NewsAgents.infrastructure import aws
...
class BusinessLogicManager(BusinessLogicInterface):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.prompt_service = self.get_prompt_service()
        self.aws_credential_provider = AWSCredentialsProvider()
        self.aws_creds = self.aws_credential_provider.get_credentials()
    ...
    def get_prompt_service(self) -> PromptService:
        ...
        if not self.prompt_service:
            ...
            if prompt_manager_type == "aws":
                from NewsAgents.infrastructure.aws.aws_prompt_manager import AWSPromptManager
    ...
    def get_workflow(self, workflow_id: str, parameters: Dict[str, Any]) -> Any:
        ...
        if workflow_id not in self.workflows:
            ...
            if workflow_id == "direct_query":
                self.workflows[workflow_id] = DirectQueryWorkflow(self.agent_manager)
            elif workflow_id == "barrons_user_feedback":
                self.workflows[workflow_id] = BarronsUserFeedBackWorkflow(self.agent_manager)
            else:
                if not self.data_manager:
                    raise ValueError("Data manager not set")
                if workflow_id == "news_query":
                    self.workflows[workflow_id] = NewsQueryWorkflow(self.agent_manager, self.prompt_service, llm, self.aws_creds)
                elif workflow_id == "stock_chat":
                    self.workflows[workflow_id] = StockDataExplainerWorkflow(self.agent_manager, self.prompt_service, llm)
                elif workflow_id == "stock_db_query":
                    self.workflows[workflow_id] = StockDBQueryWorkflow(self.agent_manager, self.prompt_service, llm, self.aws_creds)
                elif workflow_id == "barrons_user_feedback":
                    self.workflows[workflow_id] = BarronsUserFeedBackWorkflow(self.agent_manager)
                elif workflow_id == "master_chat_query":
                    self.workflows[workflow_id] = MasterChatWorkflow(self.agent_manager, self.prompt_service, llm, self.aws_creds)
                    # self.workflows[workflow_id] = MasterChatWorkflow(self.agent_manager, self.prompt_service, self.aws_creds)
                else:
