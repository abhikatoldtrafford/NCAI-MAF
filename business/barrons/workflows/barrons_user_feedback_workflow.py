...
from infrastructure.data.plugins.SQL_plugin import SQLQueryPlugin, is_valid_input
...
class BarronsUserFeedBackWorkflow(WorkflowInterface[str, str]):
    ...
    def add_update_user_feedback(self, query: str, parameters: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if feedback_id is None:
            ...
            ret = self.sql_plugin.process_query(feedback_query, input_values=(message_id, user_email, session_id, feedback_comments, feedback_type, str(preset_options)), is_update_query=True)
            ...
            ret = self.sql_plugin.process_query(feedback_query, input_values=(feedback_comments, feedback_type, str(preset_options), feedback_id, message_id, user_email, session_id))
